...
The overall purpose of the project was to demonstrate that it is possible to produce software capable of making automated reccomendations for the best locations to build new cycling infrastrucutre. This was achived though the prototype www.cyipt.bike website, which presents an interactive map of England, showing the most promising schemes and other relevant information.
...
- Identify interventions on the filtered set of barriers that would alleviate these barriers to cycling by overlying cycling potential at the desire line and route network level. A busy road parallel to cycling desire lines, for example, could result in the identification of a segregated cycle path parallel to the road. A barrier running perpendicular to cycling desire lines could lead to the addition of ‘missing links’, e.g. the creation of a route connecting two proximate cycle paths.
...
- Make the data available for practitioners, researchers and other projects in a user-friendly form, e.g. via an online Application Programming Interface (API).
...
Feedback from key stakeholders suggests that the CyIPT project results is likely to be useful in the planning process and there was high demand for a publicly accessible version of the tool.
...
That will allow the team to act on valuable feedback collected during the end of project workshop and meet the requirements of GDS to allow the CyIPT to be made publicly accessible. More detailed reccomnedations are provided at the end of this report.
...
Open Street Map (OSM) data was downloaded in bulk and a much work was put into cleaning the user-contributed tags that define road type and other features. The decison to use Open Street Map, rather an alternatives shuch as Ordnance Survey data, was due to the higher levles of detail about cycling networks provided in the OSM, and the flexible licencing terms of OSM data.
...
To overcome the road-widths challenge we turned to Ordnance Survey (OS) data. The CyIPT Team has access to a 2005 copy of the OS MasterMap for England. Code was developed to measure the width of the road polygon in the MasterMap and assign that value to the line representation of the road in the OSM.
...
Assuming this relationship found in Seville holds everywhere would be naive, not only due to cultural differences between the UK and Spain but also due to a number of measurable differences between paths constructed in one place that may not apply in another:
...
- The conditions on the rest of the route beyond the interventions are likely to play a role. For example, cycling infrastructure that allows cyclist to avoid busy roads is likely to generate more uptake than infrastructure that subsitutes for existing quiet 20 mph streets.
...
It should also be noted that CyIPT evaluates each scheme in isolation, and therefore is unable to evaluate network effects where the construction of mulitple linked schemes result in a greater increase that would be expected from each indervidual scheme. For example, An average CyIPT scheme of about 1.1 km would only increase cycling by 20%, which is to be expected as it could only benefit a small number of travellers. Ftuhermore a CyIPT use data from the PCT it only considers commutters and thus the impact on other type of jounrey is not considered.
...
Feedback from the suvery was very positive, for example all responded rate the schemes and reccomended infrastrucutre layers as useful or very useful.
...
## Outline of proposal for Phase II

In Phase I we demonstrated the advantages of a single nationwide map layer that allows transport decisions to be made at any geographic scale, without having to first select the 'region' in which work is planned to take place as the PCT does: often strategic cycling plans cover multiple regions.  This allows transport planning to reflect local travel patterns rather than being constrained by administrative or funding boundaries.
...
- Data on the location of cycling crash hotspots so decision makers can prioritise investment to make cycling safe (included in Phase I)
...
**At an estimated day rate of £500/day this would cost around £60k**.
...
- n1.  Improved collisions analysis, improvements in UI and analysis techniques, including the provision of estimated level of risk, could allow CyIPT to match and exceed the existing capabilities of services such as crash map. (30 days) 
...
- d2. Inclusion of improved traffic data, e.g. HALOGEN (30 days)
- d3. Improved road width data, replace 2005 data with current data and get permission from OS for publication (30 days)
...
- d8. The ability to display crowd-sourced data on issues with current infrastructure (20 days)
...
We could overlay existing data, e.g. from the CycleScape project.
...
We do not expect all options in the above 'menu' would be taken but it's worth considering them and the relative amount of resource required to deliver each (over **£200k** for d1 --- d8, we would expect to prioritise on adding the most important extra data layers).
...
This would require a full research stage that would take longer to complete than the options outlined above and we're not proposing it as a near-term option but something to consider, e.g. linked-in with the National Transport Model team at DfT.

We could look at evaluation of walking infrastructure, this would involve adding a full walking evaluation into the existing CyIPT toolkit.
A future possibility in the longer term, after the successor of CyIPT is secure, would be to integrate CyIPT with existing transport models used by transport planning authorities.
One question that often comes up is: where would re-allocating road space be acceptable from an economic perspective? Thus a wider stage could look at the potential impact of cycling uptake and infrastructure impact on motor traffic, and hence transport-related air pollution.
