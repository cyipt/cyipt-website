...
The overall purpose of the project was to demonstrate that it is possible to produce software capable of making automated recommendations for the best locations to build new cycling infrastructure. This was achieved through the prototype www.cyipt.bike website, which presents an interactive map of England, showing the most promising schemes and other relevant information.
...
Open Street Map (OSM) data was downloaded in bulk and a much work was put into cleaning the user-contributed tags that define road type and other features. The decision to use Open Street Map, rather an alternatives such as Ordnance Survey data, was due to the higher levels of detail about cycling networks provided in the OSM, and the flexible licencing terms of OSM data.
...
- The conditions on the rest of the route beyond the interventions are likely to play a role. For example, cycling infrastructure that allows a cyclist to avoid busy roads is likely to generate more uptake than infrastructure that substitutes for existing quiet 20 mph streets.
...
Furthermore, it allowed additional variables such as the % of the trip after an intervention that could be made on roads with low speed limits.
It was found that low speed limits of the route were a powerful predictor of cycling uptake following new infrastructure.
To model this the % of the route at different speed limits was used as an *interaction variable* that diminished the effectiveness of new infrastructure for fast (40 mph+) roads and increased the effectiveness of new infrastructure in cases where the % of the route that was on low speed limit roads was high.
...
A drop-down menu was used to filter-out schemes depending on their associated BCRs.
...
It should also be noted that CyIPT evaluates each scheme in isolation, and therefore is unable to evaluate network effects where the construction of multiple linked schemes result in a greater increase that would be expected from each individual scheme. For example, An average CyIPT scheme of about 1.1 km would only increase cycling by 20%, which is to be expected as it could only benefit a small number of travellers. Furthermore, a CyIPT use data from the PCT it only considers commuters and thus the impact on other types of journey is not considered.
...
Feedback from the survey was very positive, for example, all responded rate the schemes and recommended infrastructure layers as useful or very useful.
...
We could look at the evaluation of walking infrastructure, this would involve adding a full walking evaluation into the existing CyIPT toolkit.
