...
It should also be noted that CyIPT evaluates each scheme in isolation, and therefore is unable to evaluate network effects where the construction of multiple linked schemes result in a greater increase that would be expected from each individual scheme. For example, An average CyIPT scheme of about 1.1 km would only increase cycling by 20%, which is to be expected as it could only benefit a small number of travellers. Furthermore, a CyIPT use data from the PCT it only considers commuters and thus the impact on other types of journey is not considered. A further current limitation, which further work could address, is that CyIPT may propose two nearby schemes independently, where only one would in practice be needed due to their proximity.
